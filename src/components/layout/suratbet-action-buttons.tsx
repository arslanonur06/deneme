...
type SuratbetActionButtonsProps = {
  /** inline: tek satır kaydırma; wrap: çok satır; compact: küçük padding */
  variant?: "default" | "compact" | "hero";
  className?: string;
};

function SuratbetActionButtonsInner({
  href,
  variant = "default",
  className = "",
}: SuratbetActionButtonsProps & { href: string }) {
  const solid =
    variant === "compact"
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Suratbet ortak bağlantıları"
    >
...
export function SuratbetActionButtons(props: SuratbetActionButtonsProps) {
  const href = usePartnerAffiliateHref();
  return <SuratbetActionButtonsInner {...props} href={href} />;
}

/** `useSearchParams` yüklenene kadar kullanılan statik href (Suspense fallback). */
export function SuratbetActionButtonsFallback(props: SuratbetActionButtonsProps) {
  return (
    <SuratbetActionButtonsInner
      {...props}
